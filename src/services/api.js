...
export const planosAPI = {
  ...
  // ─── NOVO: gera o PDF e devolve blob + nome, sem baixar ────────────────────
  // Usado pelo modal de pré-visualização.
  // Quem chamar fica responsável por revogar a URL (URL.revokeObjectURL).
  gerarPDFBlob: async (payload) => {
    const res = await api.post("/api/planos/pdf/download", payload, {
      responseType: "blob",
    });
  ...
  // Download direto (mantido para compatibilidade e para o botão "Baixar")
